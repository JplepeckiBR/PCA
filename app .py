...
image_dir = "raw_data/images of people i know/test1.jpg/"
...
plt.imshow(hair)
plt.show()
left_brow = np.dstack((R*(np.array(foreground)==6),G*(np.array(foreground)==6) ,B*(np.array(foreground)==6)))
right_brow = np.dstack((R*(np.array(foreground)==7),G*(np.array(foreground)==7) ,B*(np.array(foreground)==7)))
...
left_eye = np.dstack((R*(np.array(foreground)==4),G*(np.array(foreground)==4) ,B*(np.array(foreground)==4)))
right_eye = np.dstack((R*(np.array(foreground)==5),G*(np.array(foreground)==5) ,B*(np.array(foreground)==5)))
eyes = left_eye + right_eye
plt.imshow(eyes)
plt.show()
lip1 = np.dstack((R*(np.array(foreground)==12),G*(np.array(foreground)==12) ,B*(np.array(foreground)==12)))
lip2 = np.dstack((R*(np.array(foreground)==11),G*(np.array(foreground)==11) ,B*(np.array(foreground)==11)))
lips = lip1 + lip2
plt.imshow(lips)
plt.show()
...
#######----------------------------------------------------------------------#######
#######----KNN Model for finding which tone is closest in distance-----------#######
#######---------------------------------------------------------------------#######
...
#######------------------------------------------------------------------#######
#######-----------Dataset for Facial Feature classifier(seasonal)--------------#######
#######------------------------------------------------------------------#######
...
# Example dataset, each row is [eye_R, eye_G, eye_B, hair_R, hair_G, hair_B, lip_R, lip_G, lip_B, brow_R, brow_G, brow_B]
data = np.array([
             ###-----Winter-------###
...
# Number of data points for each season
num_points_per_season = 15

# Constructing the labels array
labels = np.array([
    # Winter (label = 0)
    0] * num_points_per_season +

    # Summer (label = 1)
    [1] * num_points_per_season +

    # Autumn (label = 2)
    [2] * num_points_per_season +

    # Spring (label = 3)
    [3] * num_points_per_season
)
...
print(classification_report(y_test, y_pred, target_names=[label_mapping[i] for i in range(4)]))
...
# Assuming 'scaler' is your previously fitted StandardScaler
...
#######------------------------------------------------------------------#######
#######-----------Dataset for palette recommender------------------------#######
#######------------------------------------------------------------------#######
...
def color_palette_recommendation(predicted_classification, predicted_season):
    """
...
#######------------------------------------------------------------------#######
#######-----------visualizing for palette recommender--------------------#######
#######------------------------------------------------------------------#######
...
recommended_palette = color_palette_recommendation(predicted_classification, predicted_season)

# Print the classifications for context
print(f"The classification for facial features is: {label_mapping[prediction[0]].capitalize()}")
print(f"The classification for the skin tone is: {predicted_classification[0].capitalize()}")

# Visualize the recommended palette
visualize_color(recommended_palette, f"Recommended {predicted_classification[0].capitalize()} {label_mapping[prediction[0]].capitalize()}")
